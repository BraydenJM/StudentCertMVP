...
public class tracker
{
    ...
    /*private readExcel(string filepath)
    {
        ...
        private List<List<String>> generateFromExcel(IExcelDataReader reader)
        {
            ...
            bool isAAST = false;
            int classTracker = 0;
            while (classTracker < ds.Tables.Count)
            {
                if (ds.Tables[classTracker].TableName == "AAST")
                {
                    isAAST = true;
                    break;
                }
            ...
            if (isAAST == true)
            {
                for (int j = 0; j < ds.Tables[classTracker].Columns.Count; j++)
                {
                    List<string> col = new List<string>();
                    for (int k = 0; k < ds.Tables[classTracker].Rows.Count; k++)
                    {
                        col.Add(ds.Tables[classTracker].Rows[k][j].ToString());
                    }
                    result.Add(col);
                }
            }
    ...
    {
        foreach (char s in newSchedule)
        {
            char[] seperatorChars = { ',', '.', ':', '\t' };
            string[] classes = newSchedule.Split(seperatorChars);
            parseForClasses(classes, classes);
        }
    }
